  ...
  export default {
    ...
    computed: {
        filter: function() {
          ...
          for (var i in this.data) {
            if ( ( this.data[i].domain.match(domain_re) && 
            this.search_query.status.includes(this.data[i].status ) ) || ((this.search_query.status == "") && ( this.data[i].domain.match(domain_re)) )) 
            {
    ...
    methods: {
      getLengthStatus(items){
            var statusLength = this.data.filter((item) => item.status == items);
    ...
    mounted(){
      var vm = this;
      vm.$nextTick(()=>{
        vm.data = vm.$store.state.services.data
      })
    }
  }
</script>
